$(function() {
    ...
    $(".js-dashboard__navigation").click(function(){
        var id = $(this).attr('data-navi');

        $(".js-dashboard__navigation").removeClass("done-navi__item_active");
        $(this).addClass("done-navi__item_active");

        $(".js-navi__block").each( function() {
                if ($(this).attr('data-navi') == id) {
                    $(this).show();
                } else {
                    $(this).hide();
                }
    ...
    var taskVM = function(data) {
        ...
        this.updateTask = function(action) {
            ...
            switch (action) {
                case 'assignee': data.assignee = self.users().assignee; break;
                case 'task': data.task = self.task; break;
                case 'ticket': data.ticket = self.ticket; break;
                case 'deadline': data.deadline = self.deadline; break;
                case 'description': data.description = self.description; break;
                ...
                case 'time': data.time = self.time; break;
            }
    ...
    var dashboardVM = function(content) {
        var self = this;
        ...
        this.created = ko.observableArray();
        ...
        this.currentStateId = ko.observable();
        this.currentStateName = ko.observable();

        this.naviState = ko.observable(2);
        ...
        this.setState = function(task_id, state_id) {
            self.loading(true);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.assigned != null) {
                        for (var task in response.assigned[1]) {
                            self.todo.push (new taskVM(response.assigned[1][task]));
                        }

                        for (var task in response.assigned[2]) {
                            self.sprint.push (new taskVM(response.assigned[2][task]));
                        }
                    }
